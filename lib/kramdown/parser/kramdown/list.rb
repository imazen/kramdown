  ...
  module Parser
    class Kramdown
      ...
      def parse_list
        ...
        last_is_blank = false
        while !@src.eos?
          cur_line = @src.current_line_number
          if last_is_blank && @src.check(HR_START)
            ...
            item = new_element(:li, nil, nil, :location => cur_line)
        ...
        list.children.each do |it|
          temp = new_element(:temp, nil, nil, :location => it.options[:location])
      ...
      def parse_definition_list
        ...
        para.children.first.value.split(/\n/).each do |term|
          el = new_element(:dt, nil, nil, :location => @src.current_line_number)
        ...
        while !@src.eos?
          ...
          if @src.scan(def_start_re)
            item = new_element(:dd, nil, nil, :location => start_line_number)
